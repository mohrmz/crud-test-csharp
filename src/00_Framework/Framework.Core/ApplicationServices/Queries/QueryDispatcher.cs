...
public class QueryDispatcher : IQueryDispatcher
{
    ...
    public Task<QueryResult<TData>> Execute<TQuery, TData>(TQuery query) where TQuery : class, IQuery<TData>
    {
        ...
        {
            _stopwatch.Stop();
            _logger.LogInformation(EventIds.PerformanceMeasurement, "Processing the {QueryType} query tooks {Millisecconds} Millisecconds", query.GetType(), _stopwatch.ElapsedMilliseconds);
        }
    }
    #endregion
}
