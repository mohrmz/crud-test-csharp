...
public class EventDispatcherDomainExceptionHandlerDecorator : EventDispatcherDecorator
{
    ...
    public override async Task PublishDomainEventAsync<TDomainEvent>(TDomainEvent @event)
    {
        ...
        {
            _logger.LogError(EventIds.DomainValidationException, ex, "Processing of {EventType} With value {Event} failed at {StartDateTime} because there are domain exceptions.", @event.GetType(), @event, DateTime.Now);
        }
